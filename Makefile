...
GOLD_TARGETS_NOP := $(GOLD_SOURCES:$(GOLD_SRC)/%.c=$(GOLD_BIN)/%_nop)
GOLD_TARGETS = $(GOLD_TARGETS_TRC) $(GOLD_TARGETS_ARC) $(GOLD_TARGETS_NOP)
...
$(GOLD_TARGETS_NOP) : $(GOLD_BIN)/%_nop:$(GOLD_SRC)/%.c $(HEADERS)
	@mkdir -p $(@D)
	$(CC) $(GOLD_CFLAGS) -DGC_NOP $< -o $@
